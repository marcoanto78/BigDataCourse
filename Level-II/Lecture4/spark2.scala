...
create table fstore2 (trans_id bigint, store_id int, product_id int, loyalty_member_id int, promo_code_id int, emp_id int, amount double, qty bigint, tx_date string) row format delimited fields terminated by ',';
...
sqlContext.sql("insert into retail.fstore2 select * from temptable")

sqlContext.sql("insert into retail.fstore3 partition (tx_date='2018-01-01') select * from retail.fstore2")

sqlContext.sql("set hive.exec.dynamic.partition.mode=nonstrict")
sqlContext.sql("insert into retail.fstore3 partition (tx_date) select * from retail.fstore2")

--------------------------

HIVE:

invalidate metadata ;  -- Or refresh fstore

select * from fstore ;

a.select(tid(a("tx_date"), a("store_id"), a("lane"), a("seq_num")).alias("trans_id")).show

a.select(tid(a("tx_date"), a("store_id"), a("lane"), a("seq_num")).alias("trans_id"), a("store_id"), a("Product_id"), a("loyalty_member_id"), a("promo_code_id"), a("emp_id"), a("amount"), a("qty"),a("tx_date")).show

---------------------------
-- Create an external partitioned table
create external table fstore3 (trans_id bigint, store_id int, product_id int, loyalty_member_id int, promo_code_id int, emp_id int, amount double, qty bigint) partitioned by (tx_date string) row format delimited fields terminated by ',';

-- Insert into the partitioned table from a non partitioned table
insert into fstore3 (trans_id, store_id, product_id, loyalty_member_id, promo_code_id, emp_id, amount, qty) partition (tx_date) select * from sftore2;

-- After dropping the table, the directory still exists because the table is external
drop table fstore3;  

-- Recreate the table 
create external table fstore3 (trans_id bigint, store_id int, product_id int, loyalty_member_id int, promo_code_id int, emp_id int, amount double, qty bigint) partitioned by (tx_date string) row format delimited fields terminated by ',';

-- Must add partition in order to do a Select again
alter table fstore3 add partition (tx_date = '2018-01-01');

insert into fstore3 (trans_id, product_id, store_id, loyalty_member_id, promo_code_id, emp_id, amount, qty) partition (tx_date) select * from fstore2;
